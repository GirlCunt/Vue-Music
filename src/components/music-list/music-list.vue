  ...
  export default {
    ...
    methods:{
      handlePlaylist(playList,miniHeight){
        ...
        this.$refs.list.$el.style.bottom = playList.length > 0 ? miniHeight : ''
    ...
    watch:{
      scrollY(newY){
        let transY =  Math.max(this.minTranslateY,newY)
        let scale = 1 
        
        this.$refs.layer.style['transform'] = `translate3d(0,${transY}px,0)`
        this.$refs.layer.style['webkitTransform'] = `translate3d(0,${transY}px,0)`
        let zIndex = 0
        ...
        this.$refs.layer.style['backdrop-filter'] = `blur(${blur}px)`
